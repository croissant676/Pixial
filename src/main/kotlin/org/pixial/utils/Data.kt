...
fun Application.checkHoconDeletionStatus() {
	val deletion = this.config["pixial.development.delete"]
	val shouldDelete = kotlin.runCatching {
		deletion?.listValue?.isNotEmpty() ?: false
	}.getOrDefault(false)
	if (!shouldDelete) return
	launch {
		val names = deletion!!.listValue
		names.forEach {
			val collection = database.getCollection<Any>(it)
			collection.deleteMany()
			developmentLogger.debug { "Deleted all elements of collection $names" }
		}
	}
}